# g01.006 Adopt Published Poodle 0.4.2
...
nightfire moves its Poodle pins onto the current published release, `0.4.2`,
...
`@inflatable-cookie/poodle-core` / `-svelte` at `0.2.2` in `package.json`.
This is a two-major jump (0.2 → 0.4), so expect more migration than the 0.3 → 0.4 peers.
...
1. Move every `@inflatable-cookie/poodle-core` and `@inflatable-cookie/poodle-svelte`
   pin from `0.2.2` to `0.4.2` in `package.json`.
