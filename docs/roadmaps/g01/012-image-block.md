...
- **Presentation direction:** semantic markup, `data-nightfire-block="image"`, the sizing preset as a
  data attribute, and the sizing token property on the root. No scoped styles, no class of our own.
...
- **Scenario oracle:** register a source, pick an image, add alt text and `sizing="medium"`, save,
  reload, and confirm the reference round-trips and the renderer emits the token property plus the
  sizing data attribute. Then unregister the source and confirm the block renders inert rather than
...
4. Add the sizing preset names to the renderer's token surface, and record that adding or renaming a
...
Stop and report if the sizing presets are still unconfirmed, if the registry cannot serve both blocks
without a second registration, if a resolved reference cannot be read synchronously, or if Poodle's
image node cannot be driven from the registry. Do not add a width in pixels, do not store a URL in
block data, and do not vendor an image picker.

## Evidence

On completion, record: the registered parts, the registry extension, the preset names and their token
names, the intrinsic-dimension behaviour, the host-function path into rich text, the component tests,
